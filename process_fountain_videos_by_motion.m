...
if true
    ...
    function obj = setupSystemObjects(fpath)
        ...
        % Create System objects used for reading video, detecting moving objects,
        % and displaying the results.
        % %    obj = setupSystemObjects(); (currently in the lines below)

        % Create a video file reader.
        obj.reader = vision.VideoFileReader(fpath);

        % Create two video players, one to display the video,
        % and one to display the foreground mask.
        obj.maskPlayer = vision.VideoPlayer('Position', [740, 400, 512, 512]);
        obj.videoPlayer = vision.VideoPlayer('Position', [20, 400, 512, 512]);
         ...
         obj.detector = vision.ForegroundDetector('NumGaussians', 5, ...
            'NumTrainingFrames', 1000, 'MinimumBackgroundRatio', 0.7);
    ...
    function [centroids, bboxes, mask] = detectObjects(frame)
        ...
        mask = imopen(mask, strel('rectangle', [3,3])); %original
        mask = imclose(mask, strel('rectangle', [15, 15])); %original
%         mask = imopen(mask, strel('disk', [3])); %~BP
%         mask = imclose(mask, strel('disk', [15])); %~BP
...
%             kalmanFilter = configureKalmanFilter('ConstantVelocity', centroid, [200, 50], [100, 25], 100); %original
            
            kalmanFilter = configureKalmanFilter('ConstantAcceleration', centroid, [200, 50, 100], [100, 25, 100], 100);
